...
private import core.checkedint: mulu, addu, subu, subs, adds;
...
if (isDecimal!D)
{
    ...
    else
    {
        ...
        if (coefficient == 0U)
            return maskTest(x.data, D.MASK_SGN) ? DecimalClass.negativeZero : DecimalClass.positiveZero; 
        exponent = cast(uint)((x.data & D.MASK_EXP1) >>> D.SHIFT_EXP1);
    }

  

    bool isNegative = maskTest(x.data, D.MASK_SGN);
...
template isDataStore(T)
{
    enum isDataStore = is(T: uint) || is(T:ulong) || is(T: uint128);
}

template isUnsignedEx(T)
{
    enum isUnsignedEx = isUnsigned!T || is(T: uint128);
}
...
template unsignedPrecision(T)
{
    enum unsignedPrecision = 9 * (T.sizeof * 8) / 32 - 2;
}

bool maskTest(T, U)(auto const ref T x, auto const ref U mask)
{
    return cast(bool)(x & mask);
}
...
ExceptionFlags exp2to10(ref uint coefficient, ref int exponent)
{
...
public
@nogc @safe pure nothrow
ExceptionFlags exp2to10(ref ulong coefficient, ref int exponent)
{
...
@nogc @safe pure nothrow
ExceptionFlags exp2to10(ref uint128 coefficient, ref int exponent)
{
...
//ExceptionFlags adjustPrecision(T)(ref T coefficient, ref int exponent, const int precision, 
...
//out
//{
//    assert (prec(coefficient) <= precision);
//}
//body
//{
//    return adjustPrecision(coefficient, exponent, int.min, int.max, precision, isNegative, mode);
//}
//
////adjusts coefficient to max specified precision, watch for exponent to not overflow
//void adjustPrecisionUp(T)(ref T coefficient, ref int exponent, const int precision)
//if (isAnyUnsigned!T)
//in
//{
//    assert (precision >= 1);
...
//body
//{
//    auto maxPrecisionShift = exponent < 0 ? int.min - exponent : int.max;
//    auto targetPrecision = precision > maxPrecisionShift ? maxPrecisionShift : precision;
//    auto currentPrecision = prec(coefficient);
//    if (currentPrecision < targetPrecision)
//    {
//        auto power10 = targetPrecision - currentPrecision;
//        int maxPower10 = pow10!T.length - currentPrecision;
//        auto maxCoefficient = maxmul10!T[$ - currentPrecision];
//        if (coefficient > maxCoefficient)
//            --maxPower10;
//        power10 = power10 > maxPower10 ? maxPower10 : power10;
//        if (power10 >= 1)
//        {
//            coefficient *= pow10!T[power10];
//            exponent -= power10;
//        }
//    }
//}
...
void expandCoefficient(T)(ref T coefficient, ref int exponent)
{
    if (coefficient)
    {
        auto precision = prec(coefficient);
        auto maxPrecision = cast(int)pow10!T.length;
        auto precisionChange = maxPrecision - precision;
        auto maxExponentChange = exponent < 0 ? int.min - exponent: int.max;

        auto p = precisionChange < maxExponentChange ? precisionChange : maxExponentChange;

        if (p > 0)
        {
            auto c = coefficient;
            if (mulpow10(c, p) & ExceptionFlags.overflow)
            {
                mulpow10(c, p - 1);
                exponent -= p - 1;
            }
...
if (isSomeChar!C)
{
    ...
    Unqual!T c = coefficient;
    int digits = prec(coefficient);
    int e = digits == 0 ? 0 : exponent + (digits - 1);    
    int requestedDecimals = spec.precision == spec.UNSPECIFIED ? 6 : spec.precision;
...
if (isSomeChar!C)
{
    ...
    else
    {
        ...
        sinkExponential(espec, sink, coefficient, exponent, signed, mode, false);
    }
...
if (isDecimal!D)
{
    ...
    auto flags = coefficientAdjust(coefficient, exponent, 0, D.EXP_MAX, D.COEF_MAX, isNegative, mode);
    return x.adjustedPack(coefficient, exponent, isNegative, precision, mode, flags);
}
...
if (isDecimal!D)
{
    ...
    else
    {
        ...
        if (isNegative)
        {
            ...
            ++coefficient;
        return x.adjustedPack(coefficient, exponent, isNegative, RoundingMode.towardZero, flags);
    }
...
if (isDecimal!D)
{
    ...
    do
    {
        ++i;
        Unqual!D p = g;
        flags |= decimalDiv(p, i, 0, mode);
        flags |= decimalMul(f, p, 0, mode);
        l = x;
        flags |= decimalAdd(x, f, 0, mode);

    } while (abs(l - x) > D.epsilon);
...
ExceptionFlags decimalAtanh(D)(ref D x, const int precision, const RoundingMode mode)
{
    ...
    Unqual!D x1 = x;
    Unqual!D x2 = -x;
    auto flags = decimalAdd(x1, 1U, 0, mode);
    flags |= decimalAdd(x2, 1U, 0, mode);
    flags |= decimalDiv(x1, x2, 0, mode);
    x = x1;
    flags |= decimalLog(x, 0, mode);
    return flags | decimalDiv(x, 2U, precision, mode);
}
...
ExceptionFlags coefficientCbrt(T)(ref T cx, ref int ex)
{
    ...
    auto flags = coefficientDiv(cn, en, sn, Unqual!T(3), 0, false, RoundingMode.implicit);
    ...
    while (!coefficientEqu(cn, en, false, cl, el, false))
    {
        ...
        //x = (2x + N/x2)/3
        flags |= coefficientSqr(cn2, en2);
        flags |= coefficientDiv(cy, ey, sn, cn2, en2, false, RoundingMode.implicit);
        flags |= coefficientMul(cn, en, sn, 2, 0, false, RoundingMode.implicit);
        flags |= coefficientAdd(cn, en, sn, cy, ey, false, RoundingMode.implicit);
        flags |= coefficientDiv(cn, en, sn, Unqual!T(3), 0, false, RoundingMode.implicit);
    }
...
ExceptionFlags coefficientHypot(T)(ref T cx, ref int ex, auto const ref T cy, const int ey)
{
    ...
    auto flags = coefficientMul(cz, ez, sz, cx, ex, false, RoundingMode.implicit);
    
    cx = cy;
    ex = ey;

    flags |= coefficientMul(cx, ex, sz, cy, ey, false, RoundingMode.implicit);
    flags |= coefficientAdd(cx, ex, sz, cz, ez, false, RoundingMode.implicit);
...
ExceptionFlags coefficientPow(T)(ref T cx, ref int ex, ref bool sx, const int p)
{
...
ExceptionFlags coefficientExp(T)(ref T cx, ref int ex, ref bool sx)
{
    ...
    auto flags = coefficientAdd(cx, ex, sx, T(1U), 0, false, RoundingMode.implicit);
    ci = 1U;
    ei = 0;
    si = 0;
    ...
    do
    {
        flags |= coefficientAdd(ci, ei, si, T(1U), 0, false, RoundingMode.implicit);
        ct = cp;
        et = ep;
        st = sp;

        flags |= coefficientDiv(ct, et, st, ci, ei, si, RoundingMode.implicit);
        flags |= coefficientMul(cf, ef, sf, ct, et, st, RoundingMode.implicit);

        cl = cx;
        el = ex;
        sl = sx;

        flags |= coefficientAdd(cx, ex, sx, cf, ef, sf, RoundingMode.implicit);
    }
    while (!coefficientEqu(cl, el, sl, cx, ex, sx));
...
ExceptionFlags coefficientLog(T)(ref T cx, ref int ex, ref bool sx)
{
    ...
    {
        enum uint128 ce = unit128("271828182845904523536028747135266249776");
        enum int ee = -38;
    }
    else
        static assert(0);

    Unqual!T cn, cp, cl, cr, ca;
    int en, ep, el, er, ea;
    bool sn, sp, sl, sr, sa;
    ...
    while (coefficientCmp(cp, ep, sp, ce, ee, se) >= 0)
    {
        flags |= coefficientDiv(cp, ep, sp, ce, ee, false, RoundingMode.implicit);
        flags |= coefficientAdd(cn, en, sn, T(1U), 0, false, RoundingMode.implicit);
    }   

    flags |= coefficientDiv(cp, ep, sp, ce, ee, false, RoundingMode.implicit);
    flags |= coefficientAdd(cn, en, sn, cp, ep, sp, RoundingMode.implicit);
    ...
    do
    {
        ...
        flags |= coefficientAdd(cn, en, sn, T(1U), 0, true, RoundingMode.implicit);
        flags |= coefficientExp(cn, en, sn);
        flags |= coefficientDiv(cp, ep, sp, cn, en, sn, RoundingMode.implicit);

    } while (!coefficientEqu(cn, en, sn, ca, ea, sa));
